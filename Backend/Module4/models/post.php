...
class Post extends Model
{
    ...
    public function getAnalitic($count = 0)
    {
        if ($count){
    ...
    public function getAnaliticCount()
    {
        $sql = "
            select COUNT(n.id) as count from news n
            WHERE n.is_analitic = 1
            ";
        return $this->db->query($sql);
    }

    public function searchByTag($keywords)
    {
    ...
    public function getByCategory($category, $count = 0)
    {
    ...
    public function getTopPost()
    {
        $sql = "
               SELECT COUNT(news.news_tittle) as total, news.id, news.news_tittle, news_comment.date as comment_date FROM news
                JOIN news_comment
                ON news.id = news_comment.id_post AND news_comment.is_publish != 0
                GROUP BY (news.news_tittle)
                ORDER BY COUNT(news_comment.id) DESC 
                LIMIT 5
               ";
        return $this->db->query($sql);
    }

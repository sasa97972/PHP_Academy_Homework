...
{
    public function save($data, $isPublish = 1)
    {
        if (!empty($data)){
            ...
            $sql ="
                INSERT INTO news_comment (id_user, id_post, content, is_publish) VALUES ({$user}, {$post}, '{$content}', {$isPublish})
                  ";
        }
    ...
    public function getByPost($id = null)
    {
        if ($id != null){
            $sql = "
                  ...
                  users u
                  ON c.id_user = u.id
                  WHERE c.id_post = {$id} AND c.is_publish = 1
    ...
    public function getById($id = null)
    {
        if ($id != null){
            $sql = "
                  SELECT c.id, c.content, c.date, c.positive, c.negative  FROM news_comment c
                  WHERE c.id = {$id}
                   ";
        }
        return $this->db->query($sql);
    ...
    {
        $sql = "
           DELETE FROM `news_comment` WHERE `news_comment`.`id` = {$id}
        ";
        return $this->db->query($sql);
    }
    ...
    public function updateComment($id, $text)
    {
        ...
        $sql = "
           UPDATE `news_comment` SET `content` = '{$text}' WHERE `id` = {$id}
        ";
        return $this->db->query($sql);
    }

    public function getPoliticsComments()
    {
        $sql = "
               SELECT u.login AS user, n.news_tittle, n.id AS id_news, c.id, c.content, c.date, c.positive, c.negative, c.date  FROM news_comment c
              JOIN
    ...
    {
        $sql = "
           UPDATE `news_comment` SET `is_publish` = 1 WHERE `id` = {$id}
        ";
        return $this->db->query($sql);
    }
}
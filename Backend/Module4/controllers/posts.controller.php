...
class PostsController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($this->data['category'] as $category) {
            $this->data['posts'][$category['category']] = $this->model->getByCategory($category['category']);
        }
    ...
    public function search()
    {
        $arr = array();
            if (!empty($_POST['keywords'])) {
                $arr['names'] = $this->model->searchByName($_POST['keywords']);
                $resultTag = $this->model->searchByTag($_POST['keywords']);
                if($resultTag) {
                    $arr['tags'] = explode(',',$resultTag[0]['tag']);
                } else {
                    $arr['tags'] = [];
                }
            }
    ...
    public function category()
    {
        ...
        $this->data['uri'] = Helper::getBasePath().App::getRouter()->getController()."/".App::getRouter()->getAction()."/".$params[0];
    ...
    public function analitic()
    {
        ...
        $this->data['analitic'] = $this->model->getAnalitic($page);
        $this->data['uri'] = Helper::getBasePath().App::getRouter()->getController()."/".App::getRouter()->getAction();
        $this->data['count'] = current(current($this->model->getAnaliticCount()));
    }
